...
rewiring <- function(nets){
  ...
  graph0[2:length(graph)] <- lapply(2:length(graph), function(i) {
    as.matrix(rewire_graph(graph[[i]], p = 20 * nties[i], algorithm = "swap"))
    })
  
  graph0 <- blockdiagonalize(graph0)
  
  graph0 <- new_barry_graph(as.matrix(graph0), n = length(nets))
...
bootstrapping <- function(nsim = 1000, nets = nets, ncores = 4) {
  ...
  parallel::clusterExport(cl, c("nets"), envir = environment())
  parallel::clusterExport(cl, "rewiring", envir = .GlobalEnv)
...
fr.ans <- bootstrapping(nsim = 2000,
                        nets = nets)

# advice
nets <- c(list(ad.true), advice_nets)
ad.ans0 <- emp_count(nets = nets)
ad.ans <- bootstrapping(nsim = 100,
                        nets = nets)
```
